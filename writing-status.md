...
- `Header::write` mirrors the layout of the 0x70-byte header, and `build_dex_file_bytes` now patches in the final file size, SHA-1 signature, and Adler-32 checksum once all sections are laid out.
...
- `ClassDataItem` emission still needs additional validation around overflowing try ranges/handler offsets, but concrete methods (code + debug info) now flow through the builder.
### Missing Functionality
- `ClassDefItem::write` remains a thin helper that expects precomputed offsets; the layout plan now supplies interfaces/static-values/class-data/annotation-directory offsets, but true link-section data and non-literal static blobs (e.g. encoded arrays referenced outside field initialisers) are still missing.
- Index pools exist (`DexIndexPools`) and can output ID-table payloads. Literal harvesting now parses method-handle descriptors (methods + fields) and call-site descriptors so bootstrap methods, target names, and their prototypes land in the canonical pools ahead of time, and it canonicalises string/array literals discovered while assembling bytecode; however, we still need to extend this to cover future link/call-site sections once they’re emitted.
- Call-site/method-handle section support and link-section emission are still missing.
- Debug info generation now honors `.line`, `.prologue`, `.epilogue`, `.local`, `.end local`, and `.restart local` directives, producing the corresponding Dalvik debug opcodes (`DBG_START_LOCAL{,_EXTENDED}`, `DBG_END_LOCAL`, `DBG_RESTART_LOCAL`, etc.). Future work includes supporting `.set file` and richer local scope transitions (e.g., overlapping names, parameter renames mid-method).
- `DexLayoutPlan` now emits real `CodeItem`s, but we still need to stress-test enormous try/catch tables, verify map list coverage for every chunk (annotations, code, debug info), and plug in optional sections such as link data once their producers exist.

### Instruction Format Coverage Snapshot
- ✅ Implemented: `Format10x`, `11n`, `11x`, `12x` (move/move-object/move-wide + arithmetic/convert ops), `20bc`, `21s`, `21ih`, `21lh`, `21c`, `21t`, `22b`, `22c`, `22cs`, `22s`, `22t`, `22x`, `23x`, `31i`, `31t`, `31c`, `32x`, `35c`, `35mi`, `35ms`, `3rc`, `3rmi`, `3rms`, `45cc`, `4rcc`, `51l`, all branch encodings (`10t/20t/30t`), and payload helpers for `.array-data`, `.packed-switch`, and `.sparse-switch`.
- ⏳ Remaining: No uncovered formats remain in the current scope. Future odex-only oddities (e.g., `invoke-direct-empty`, barrier returns) reuse already-supported formats and can be added as new `DexOp` variants if we decide to expose them.
